...
/* ========== Clerk Helper (robust, aber optional) ========== */
function getClerk() {
...
export default function Home() {
    ...
    (async () => {
      ...
      const normalized = raw.map((p) => {
        ...
        return {
          ...
          category,
          sizes,
          // Preis wird zur Anzeige/Cart später mit getPrice() gezogen
        };
      });
  ...
  // ▼ NEU: einheitliche Preisfunktion (nutzt Overrides, sonst Fallback)
  function getPrice(p) {
    // Versuche Key so wie er im Shop vorkommt + Varianten (decoded/encoded)
    const k1 = p.image;
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [items, active, sort, q, priceMap]); // sortiert neu, wenn Preise geladen sind
  ...
  function addToCart(p, idx) {
    ...
    const price = getPrice(p); // ▼ NEU: derselbe Preis wie angezeigt
    setCart((prev) => {
      ...
      <div style={{ position:'sticky', top:0, zIndex:50, backdropFilter:'blur(6px)', background:'rgba(0,0,0,.45)', borderBottom:'1px solid #1b253b' }}>
        <div className="container" style={{ height:56, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            ...
            {!signedIn ? (
              <>
                <button className="neon" onClick={() => safeOpenClerk('signIn')}>Login</button>
                <button className="neon" onClick={() => safeOpenClerk('signUp')}>Registrieren</button>
              </>
            ) : (
              <>
                <button
          ...
          {filtered.map((p, idx) => (
            <div className="card" key={p.image + idx}>
              ...
              <div style={{ padding: 12 }}>
                ...
                {/* ▼ NEU: Anzeige verwendet getPrice() */}
